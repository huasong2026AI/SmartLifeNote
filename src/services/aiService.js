// AI Batch Processing Service with Multimodal Audio/Vision and 3-Tier Categorization Hierarchy
...
重要：如果随手记中附带了录音文件（音频部分），请仔细聆听并转写录音内容，合并到该条记录的文本中进行深度归类与分析。如果附带了拍照图片，请识别并提取图片中的事物与文字内容，合并到文本中分析。
...
2. 【二级小类/主题 (sub_category)】：在大类下细分具体主题或病程 Timeline（如: 工作事业 -> "项目开发"；家庭生活 -> "汽车保养"；学习提升 -> "日语学习"；求医问诊 -> "一次感冒发烧病程"）。
3. 【三级关联内容】：
   - 包含具体随手记条目 (records)
          ...
          "records": [
            {
              ...
              "event_time": "发生时间",
              "object": "事件/关键对象",
              "content_summary": "包括录音转写和图片识别后的完整记录内容精炼摘要"
            }
          ],
...
export const aiService = {
  ...
  callGeminiAPI: async (apiKey, modelName, notes) => {
    ...
    const parts = [
      { text: SYSTEM_PROMPT },
      { text: `以下是用户录入的 ${notes.length} 条随手记。如果记录中包含图片或录音多媒体数据，请仔细提取图片细节/文字以及听写录音内容，合并汇总后完成三级目录归类与预测总结。` }
    ];
    ...
    notes.forEach((n, index) => {
      ...
      // Add attached image if present (multimodal image part)
      ...
      // Add attached recorded audio if present (multimodal audio part)
      if (n.audioBase64) {
        parts.push({
          inlineData: {
